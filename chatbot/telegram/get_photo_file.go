...
func GetPhotoFile(tgPhotoSize *models.TGPhotoSize) (body []byte, err error) {
	start := time.Now()

	// Check if we've retrieved file already
	if tgPhotoSize.FileLocation.Valid {
		b, err2 := files.GetBytes(fmt.Sprintf("chatbot/telegram/%s", tgPhotoSize.FileLocation.String))
		if err2 == nil {
			body = *b
